...
Функционал: Создание игры, возможность обновит счёт, получить список вариантов ответов, начать следующий раунд...
    ...
    Сценарий: Правильный ответ игрока
        Дано идущая игра, ход игрока 1
        Когда игрок даёт правильный ответ и нажимает на кнопку ответить
        Тогда Счёт игры изменяется

    Сценарий: Неправильный ответ игрока
        Дано идущая игра, ход игрока 1
        Когда игрок даёт неправильный ответ и нажимает на кнопку ответить
        Тогда Счёт игры не изменяется и ход переходит к игроку 2

    Сценарий: Правильный ответ игрока и завершение игры
        Дано идущая игра, ход игрока 1
        Когда игрок даёт правильный ответ и нажимает на кнопку ответить, а затем завершает игру
        Тогда Счёт игры изменяется, игрок 1 занимает 1 место